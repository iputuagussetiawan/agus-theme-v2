...
function tmdr_print_css($name, $filePath) {
    ...
    $cssPath = get_template_directory_uri() . '/dist/css/';
    return wp_enqueue_style($name,  $cssPath . $filePath, array(), $themeVersion, 'all');
}

function tmdr_print_js($name, $filePath) {
    $themeVersion = wp_get_theme()->get('Version');
    $jsPath = get_template_directory_uri() . '/dist/js/';
    return wp_enqueue_script($name, $jsPath . $filePath, array(), $themeVersion, true);
}
...
function tmdr_add_google_fonts() {
    ...
    <link rel="stylesheet" href="' . $google_font_url . '" media="print" onload="this.media=all" />
    <noscript>
