...
if (document.querySelectorAll('.list-titles').length > 0) {
  ...
  lines.forEach(function (line) {
    return line.classList.add('line-arange');
  });
  var listImages = Array.from(document.querySelectorAll('.list-image'));
  listImages.forEach(function (element, index) {
    var slide = element.setAttribute('data-index', "slide-".concat(index + 1));
    var slideIna = element.setAttribute('data-slide', index + 1);
    var totIndex = listImages.length;
    document.querySelector('.lt-total').textContent = '0' + totIndex;
    var slideClass = element.dataset.index;
    element.classList.add(slideClass);
    window.slide = slide;
    var title = element.querySelector('.list-p-title').textContent;
    window.title = title;
  });
...
/******/ })()
;
//# sourceMappingURL=list-carousel.js.map